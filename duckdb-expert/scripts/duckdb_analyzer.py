...
class DuckDBAnalyzer:
    ...
    def analyze_file(
        ...
        result = {
            ...
            "analysis_config": {
                "sample_size_for_statistics": sample_size,
                "quality_metrics_scope": "full_data"
            }
        }
    ...
    def _get_basic_info(self, file_path: str, file_ext: str) -> Dict:
        ...
        # 列数
        col_count = len(self.con.execute(f"""
    ...
    def _get_statistics(self, file_path: str, file_ext: str, sample_size: Optional[int]) -> Dict:
        ...
        for col in schema:
            ...
            # 数値型の統計
            if any(t in col_type for t in ["INT", "FLOAT", "DOUBLE", "DECIMAL", "NUMERIC", "BIGINT", "SMALLINT", "TINYINT", "HUGEINT", "UBIGINT"]):
                try:
            ...
            # BOOLEAN型の統計
                ...
                try:
                    ...
                    col_stats.update({
                        "true_count": bool_stats[0],
                        "false_count": bool_stats[1],
                        "null_count": bool_stats[2]
                    })
                except Exception as e:
                    col_stats["error"] = str(e)
    ...
    def _get_quality_metrics(self, file_path: str, file_ext: str, schema: List[Dict]) -> Dict:
        ...
        for col in schema:
            ...
            try:
                ...
                quality["null_percentages"][col_name] = round(
                    100.0 * null_count / total_rows, 2
                ) if total_rows > 0 else 0
                quality["completeness"][col_name] = round(
                    100.0 * (total_rows - null_count) / total_rows, 2
                ) if total_rows > 0 else 0
            except Exception:
        ...
        try:
            ...
            quality["duplicate_percentage"] = round(
                100.0 * duplicate_count / total_rows, 2
            ) if total_rows > 0 else 0
    ...
    def _to_markdown(self, result: Dict) -> str:
        ...
        # ヘッダー
        md.append(f"# Data Profile Report")
        ...
        for col_name, stats in result['statistics'].items():
            ...
            if 'min_length' in stats:
                md.append(f"- **Length (min/avg/max):** {stats['min_length']} / {stats['avg_length']} / {stats['max_length']}")
            if 'true_count' in stats:
                md.append(f"- **True/False/Null:** {stats['true_count']} / {stats['false_count']} / {stats['null_count']}")
            if 'avg_list_length' in stats:
                md.append(f"- **List Length (min/avg/max):** {stats['min_list_length']} / {stats['avg_list_length']} / {stats['max_list_length']}")
            if 'note' in stats:
...
Note:
    ...
    parser.add_argument(
        "input_file",
        help="Input file path (supports wildcards like 'data/*.csv')"
    )
    parser.add_argument(
        "--output", "-o",
    ...
    parser.add_argument(
        "--sample", "-s",
        type=int,
        help="Sample size for statistics calculation (quality metrics always use full data)"
    )
    parser.add_argument(
        "--memory",
        default="4GB",
