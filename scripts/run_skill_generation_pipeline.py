...
MIN_WORK_UTILITY = 15  # skip ideas with work_utility below this threshold
...
def run_mine(project_root: Path, dry_run: bool = False) -> Path | None:
    ...
    if result.returncode != 0:
        logger.error("Mine script failed (rc=%d): %s", result.returncode, result.stderr.strip()[:500])
        return None
...
def run_score(
    ...
    if result.returncode != 0:
        logger.error("Score script failed (rc=%d): %s", result.returncode, result.stderr.strip()[:500])
        return False
...
def write_weekly_summary(
    ...
    sorted_ideas = sorted(ideas, key=lambda x: x.get("scores", {}).get("composite", 0), reverse=True)
...
def _get_staged_files(project_root: Path, skill_name: str) -> list[str]:
    """Return list of staged files under the skill directory and README.md."""
    result = subprocess.run(
    ...
    if result.returncode == 0 and result.stdout.strip():
        files.extend(f.strip() for f in result.stdout.strip().splitlines() if f.strip())
    ...
    if readme_result.returncode == 0 and readme_result.stdout.strip():
        files.extend(f.strip() for f in readme_result.stdout.strip().splitlines() if f.strip())
...
def select_next_idea(backlog: dict, project_root: Path) -> dict | None:
    ...
    Eligible = work_utility >= MIN_WORK_UTILITY AND
    ...
    for idea in backlog.get("ideas", []):
        ...
        work_utility = idea.get("scores", {}).get("work_utility", 0)
        if work_utility < MIN_WORK_UTILITY:
            logger.info(
                "Skipping %s: work_utility=%s < %s",
                idea.get("id"),
                work_utility,
                MIN_WORK_UTILITY,
            )
...
def update_backlog_status(project_root: Path, idea_id: str, status: str, pr_url: str | None = None) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(dir=str(backlog_path.parent), suffix=".yaml", prefix=".backlog_")
    try:
...
def _check_unexpected_changes(project_root: Path, skill_name: str) -> bool:
    ...
    # README.md is an exact match, not a prefix
    allowed_exact = ["README.md"]
    unexpected = [
        f.strip()
        for f in all_changes
        if f.strip() and not any(f.strip().startswith(p) for p in allowed_prefixes) and f.strip() not in allowed_exact
    ]
...
def design_skill(project_root: Path, idea: dict, skill_name: str, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        # Verify SKILL.md was actually created
...
def review_and_improve(project_root: Path, skill_name: str, dry_run: bool = False) -> tuple[bool, dict | None]:
    ...
    for iteration in range(MAX_DESIGN_ITERATIONS):
        ...
        if is_final:
            logger.warning("Final iteration score %d below threshold %d.", score, DESIGN_SCORE_THRESHOLD)
            return False, report
...
def create_skill_pr(
    ...
    pr_body = (
        ...
        f"## Post-merge TODO\n"
        f"- [ ] SKILL.md frontmatter is valid\n"
        f"- [ ] Scripts have tests\n"
        f"- [ ] README.md updated with new skill entry\n\n"
...
def _record_daily_state(
    ...
    try:
        write_daily_generation_summary(project_root, idea, skill_name or "unknown", None, pr_url, dry_run=dry_run)
    except Exception:
...
def run_daily(project_root: Path, dry_run: bool = False) -> int:
    ...
    try:
        ...
        if existing_pr_url is not None:
            ...
            _record_daily_state(project_root, idea, skill_name, idea_id, 0, existing_pr_url, "pr_already_open")
            return 0
        ...
        if result.returncode != 0:
            ...
            _record_daily_state(project_root, idea, skill_name, idea_id, 0, None, "branch_create_failed")
            return 1
        ...
        if not _check_unexpected_changes(project_root, skill_name):
            ...
            _record_daily_state(project_root, idea, skill_name, idea_id, 0, None, "unexpected_changes")
            return 1
        ...
        if not _check_unexpected_changes(project_root, skill_name):
            ...
            update_backlog_status(project_root, idea_id, "unexpected_changes")
            _record_daily_state(project_root, idea, skill_name, idea_id, 0, None, "unexpected_changes_post_improve")
            return 1
