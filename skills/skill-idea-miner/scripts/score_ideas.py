...
def _build_scoring_prompt(scorable: list[dict]) -> str:
    """Build the LLM prompt for scoring skill idea candidates."""
    ...
    return (
        ...
        "- novelty: How unique is this idea compared to typical business tools?\n"
        "- feasibility: How practical is it to implement as a Claude skill?\n"
        "- work_utility: How useful is this for business professionals?\n\n"
        ...
        "Required format:\n"
        '{"candidates": [{"id": "...", "novelty": 0, "feasibility": 0, "work_utility": 0}]}'
    )
...
def score_with_llm(candidates: list[dict], dry_run: bool = False) -> list[dict]:
    ...
    Scores each candidate on novelty, feasibility, and work_utility (0-100).
    ...
    prompt = _build_scoring_prompt(scorable)
    ...
    try:
        ...
        if parsed and "candidates" in parsed:
            ...
            for c in scorable:
                ...
                if cid in score_map:
                    ...
                    w = s.get("work_utility", 0)
                    c["scores"] = {
                        "novelty": n,
                        "feasibility": f,
                        "work_utility": w,
                        "composite": round(0.3 * n + 0.3 * f + 0.4 * w, 1),
                    }
                else:
                    c["scores"] = {
                        "novelty": 0,
        ...
        for c in scorable:
            c["scores"] = {"novelty": 0, "feasibility": 0, "work_utility": 0, "composite": 0}
...
def merge_into_backlog(backlog: dict, scored_candidates: list[dict]) -> dict:
    ...
    for candidate in scored_candidates:
        ...
        idea = {
            ...
            "scanned_projects": candidate.get("scanned_projects", []),
            "source_raw_ids": candidate.get("source_raw_ids", []),
            "scores": candidate.get("scores", {}),
            "status": candidate.get("status", "pending"),
            "created_at": candidate.get("created_at", datetime.now(timezone.utc).strftime("%Y-%m-%d")),
            "attempted_at": None,
            "pr_url": None,
        }
...
def parse_args():
    ...
    parser.add_argument(
        "--candidates",
        default="reports/skill-generation-log/raw_candidates.yaml",
        help="Path to raw_candidates.yaml",
    )
    parser.add_argument("--project-root", default=".", help="Project root directory")
    parser.add_argument(
        "--backlog",
        default="logs/.skill_generation_backlog.yaml",
