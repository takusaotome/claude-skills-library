...
class FinancialRatioAnalyzer:
    ...
    def calculate_all_ratios(self) -> List[RatioResult]:
        ...
        self._add_ratio('ROE', 'Profitability',
                       self._safe_divide(self.data.net_income, self.data.total_equity),
                       'roe')
        self._add_ratio('ROA', 'Profitability',
                       self._safe_divide(self.data.net_income, self.data.total_assets),
                       'roa')
        self._add_ratio('Gross Margin', 'Profitability',
                       self._safe_divide(self.data.gross_profit, self.data.revenue),
                       'gross_margin')
        self._add_ratio('Operating Margin', 'Profitability',
                       self._safe_divide(self.data.operating_income, self.data.revenue),
                       'operating_margin')
        self._add_ratio('Net Profit Margin', 'Profitability',
                       self._safe_divide(self.data.net_income, self.data.revenue),
                       'net_margin')

        # Safety/Liquidity Ratios
        self._add_ratio('Current Ratio', 'Safety',
                       self._safe_divide(self.data.current_assets, self.data.current_liabilities),
                       'current_ratio')
        self._add_ratio('Quick Ratio', 'Safety',
                       self._safe_divide(self.data.current_assets - self.data.inventory,
                                        self.data.current_liabilities),
                       'quick_ratio')
        self._add_ratio('Debt-to-Equity', 'Safety',
                       self._safe_divide(self.data.total_liabilities, self.data.total_equity),
                       'debt_to_equity', inverted=True)
        self._add_ratio('Interest Coverage', 'Safety',
                       self._safe_divide(self.data.operating_income, self.data.interest_expense),
                       'interest_coverage')

        # Efficiency Ratios
        self._add_ratio('Asset Turnover', 'Efficiency',
                       self._safe_divide(self.data.revenue, self.data.total_assets),
                       'asset_turnover')
        self._add_ratio('Inventory Turnover', 'Efficiency',
                       self._safe_divide(self.data.cost_of_goods_sold, self.data.inventory),
                       'inventory_turnover')
        self._add_ratio('Receivables Turnover', 'Efficiency',
    ...
    def _add_ratio(self, name: str, category: str, value: float,
                   ratio_key: str, inverted: bool = False):
    ...
    def visualize_ratios(self, output_path: Optional[str] = None):
        ...
        ax.set_ylim(0, 100)
        ax.set_title(f'Financial Health Overview\n{self.data.company} - {self.data.period}',
        ...
        ax.legend(handles=[strong_patch, acceptable_patch, weak_patch],
...
class DCFAnalyzer:
    ...
    def __init__(self, initial_investment: float, cash_flows: List[float],
                 discount_rate: float = 0.10, terminal_growth_rate: float = 0.02):
    ...
    def calculate_irr(self, guess: float = 0.10, max_iterations: int = 100,
    ...
    def generate_report(self, project_name: str = "Investment Project") -> str:
        ...
        report.append(f"| IRR | {irr:.1%} | {irr_status} - {'Exceeds' if irr > self.discount_rate else 'Below'} hurdle rate |")
...
class SensitivityAnalyzer:
    ...
    def run_sensitivity(self, variable: str, range_pct: float = 20,
    ...
    def generate_tornado_data(self, variables: List[str],
                              range_pct: float = 20) -> List[Dict]:
    ...
    def visualize_tornado(self, tornado_data: List[Dict],
...
def main():
    ...
    ratios_parser.add_argument('--visualize', '-v', action='store_true',
    ...
    dcf_parser.add_argument('--discount-rate', '-r', type=float, default=0.10,
    ...
    variance_parser.add_argument('--threshold', '-t', type=float, default=5.0,
                                 help='Materiality threshold percentage (default: 5)')
    ...
    sensitivity_parser.add_argument('--range', '-r', type=float, default=20,
                                    help='Sensitivity range percentage (default: 20)')
    sensitivity_parser.add_argument('--output', '-o', help='Output file path')
    sensitivity_parser.add_argument('--visualize', action='store_true',
                                    help='Generate tornado chart')
