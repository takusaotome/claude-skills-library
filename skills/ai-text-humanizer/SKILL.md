...
1. **テキスト受領** — ユーザーからテキストを受け取る（ファイルパス or 直接入力）
...
4. **レポート生成** — `assets/detection_report_template.md` をベースにレポートを作成
...
1. **Markdown装飾を削除** — `**太字**`、`## 見出し`、箇条書きを地の文に変換
2. **括弧を最小化** — `（補足説明）` は本文に溶け込ませるか削除
3. **ダッシュ・スラッシュを排除** — `—` `／` を自然な接続に置換
4. **過剰な句読点を整理** — `、` の連続使用を短文分割で解消
5. **引用符を自然化** — `「」` の入れ子 `『』` を避ける
6. **リスト構造を文章化** — 箇条書きを段落に変換
