...
class ITROICalculator:
    ...
    @staticmethod
    def calculate_irr(cash_flows: List[float], guess: float = 0.10,
        ...
        for _ in range(max_iterations):
            ...
            derivative = sum(-t * cf / ((1 + rate) ** (t + 1))
                           for t, cf in enumerate(cash_flows))
    ...
    def sensitivity_analysis(self, base_metrics: FinancialMetrics,
                            project_data: ProjectData) -> Dict[str, FinancialMetrics]:
        """
        ...
        for scenario_name, factors in scenarios.items():
            ...
            adjusted_benefits = {
                k: v * factors['benefit_factor']
                for k, v in project_data.annual_benefits.items()
            }
    ...
    def calculate_all_metrics(self, initial_investment: float,
                               annual_operating_cost: float,
                               annual_benefits: List[float],
                               discount_rate: float) -> FinancialMetrics:
        """
...
def generate_report(project_name: str, metrics: FinancialMetrics,
...
Examples:
  ...
  From JSON file:
    ...
    parser.add_argument('--investment', '-i', type=float,
                       help='Initial investment amount')
    parser.add_argument('--annual-benefit', '-b', type=float,
                       help='Annual benefit amount')
    parser.add_argument('--annual-cost', '-c', type=float, default=0,
                       help='Annual operating cost (default: 0)')
    parser.add_argument('--years', '-y', type=int, default=5,
                       help='Analysis period in years (default: 5)')
    parser.add_argument('--rate', '-r', type=float, default=0.10,
        ...
        result = {
            ...
            'sensitivity': {
                k: {
                    ...
                    'npv': v.npv,
                    'irr': v.irr * 100,
                    'payback_period': v.payback_period
                }
