...
1. **要件の確認**: ユーザーが実行したい操作を特定
2. **認証状態の確認**: 必要に応じて `aws sts get-caller-identity` で確認を推奨
3. **コマンド提供**: 目的に応じたCLIコマンドを提示
4. **オプション説明**: --query, --output, --profile などのオプションを解説
5. **セキュリティ考慮**: 必要に応じてセキュリティベストプラクティスを提示
