...
def kpi_card(value, title, change_pct=None, sparkline_data=None, figsize=(3, 2.5)):
    ...
    # Title
    ax.text(
        0.5, 0.92, title, ha="center", va="top", fontsize=12, fontweight="bold", transform=ax.transAxes, color="#333333"
    )
    ...
    ax.text(0.5, 0.65, value_str, ha="center", va="center", fontsize=32, fontweight="bold", transform=ax.transAxes)
    ...
    if change_pct is not None:
        ...
        color = (
            FINANCIAL["positive"]
            if change_pct > 0
            else FINANCIAL["negative"]
            if change_pct < 0
            else FINANCIAL["neutral"]
        )
        ax.text(
            0.5,
            0.42,
...
def executive_summary_template(
    kpis: List[Dict], title: str = "Executive Summary", subtitle: str = "", figsize: Tuple[int, int] = (16, 10)
) -> plt.Figure:
    """
    ...
    for i, kpi in enumerate(kpis):
        ...
        # Border
        border = plt.Rectangle((0, 0), 1, 1, fill=False, edgecolor="#CCCCCC", linewidth=2, transform=ax.transAxes)
        ax.add_patch(border)

        # Title
        ax.text(0.5, 0.88, kpi["title"], ha="center", va="top", fontsize=11, fontweight="bold", transform=ax.transAxes)
        ...
        ax.text(0.5, 0.60, value_str, ha="center", va="center", fontsize=28, fontweight="bold", transform=ax.transAxes)
...
def comparison_bar_template(
    ...
    for i, (idx, row) in enumerate(df.iterrows()):
        value = row[value_col]
        ax.text(value + max(df[value_col]) * 0.02, i, f"{value:,.0f}", va="center", fontsize=10, fontweight="bold")
...
def grouped_comparison_template(
    ...
    for i, (col, color) in enumerate(zip(pivot.columns, colors)):
        ...
        ax.bar(x + offset, pivot[col], width, label=col, color=color, edgecolor="black", alpha=0.8)
...
def trend_with_forecast_template(
    ...
    if forecast_data is not None:
        ...
        ax.text(sep_date, ax.get_ylim()[1], " Forecast →", ha="left", va="top", fontsize=10, color="gray")
...
def correlation_heatmap_template(
    correlation_matrix: pd.DataFrame, title: str = "Correlation Matrix", figsize: Tuple[int, int] = (10, 8)
) -> plt.Figure:
    """
...
def waterfall_chart_template(
    ...
    # Create bars
    bars = ax.bar(range(len(categories)), values, bottom=cumsum, color=colors, edgecolor="black", alpha=0.8)

    # Add connectors
    for i in range(len(categories) - 1):
        ax.plot([i + 0.4, i + 1 - 0.4], [cumsum[i] + values[i], cumsum[i] + values[i]], "k--", linewidth=1, alpha=0.5)
    ...
    for i, (cat, val, pos) in enumerate(zip(categories, values, cumsum)):
        ...
        ax.text(
            i,
            label_pos,
            f"{val:+.0f}" if i not in [0, len(values) - 1] else f"{val:.0f}",
            ha="center",
            va="center",
            fontweight="bold",
            fontsize=10,
...
if __name__ == "__main__":
    # Example: KPI Card
    fig1 = kpi_card(
        value=1234567, title="Monthly Revenue", change_pct=12.5, sparkline_data=[100, 105, 102, 108, 115, 120, 123]
    )
    ...
    fig3 = comparison_bar_template(
        data, "Product", "Sales", title="Product Sales Comparison", highlight_max=True, target_line=250
    )
