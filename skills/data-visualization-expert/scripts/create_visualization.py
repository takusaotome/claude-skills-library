...
def create_line_chart(
    ...
    if hue:
        ...
        df = df.sort_values(x)
        ax.plot(df[x], df[y], linewidth=2.5, color=colors[0], marker="o" if markers else None, markersize=5)
        if show_area:
...
def create_scatter_plot(
    ...
    if hue:
        ...
        ax.scatter(df[x], df[y], s=s, alpha=0.6, c=colors[0], edgecolors="black", linewidth=0.5)

    # Add trendline
    if show_trendline:
        z = np.polyfit(df[x], df[y], 1)
        p = np.poly1d(z)
        ax.plot(df[x], p(df[x]), "r--", alpha=0.8, linewidth=2, label=f"Trend: y={z[0]:.2f}x+{z[1]:.2f}")
    ...
    if plot_type in ["hist", "kde"]:
        ...
        if hue:
            for i, category in enumerate(data[hue].unique()):
                ...
                if plot_type == "hist":
                    ax.hist(
                        subset, bins=30, alpha=0.6, label=category, color=colors[i % len(colors)], edgecolor="black"
                    )
                else:  # kde
                    subset.plot(kind="density", ax=ax, label=category, color=colors[i % len(colors)], linewidth=2)
        else:
            if plot_type == "hist":
                ax.hist(data[column].dropna(), bins=30, alpha=0.7, color=colors[0], edgecolor="black")
                # Add KDE overlay
                data[column].dropna().plot(kind="density", ax=ax, color="red", linewidth=2, label="KDE")
            else:  # kde
                data[column].dropna().plot(kind="density", ax=ax, color=colors[0], linewidth=2)
...
def create_dashboard(
    ...
    for i, kpi in enumerate(kpis):
        ...
        # KPI name
        ax.text(0.5, 0.85, kpi["name"], ha="center", va="top", fontsize=12, fontweight="bold", transform=ax.transAxes)

        # KPI value
        value_str = f"{kpi['value']:,.0f}" if kpi["value"] >= 1 else f"{kpi['value']:.2f}"
        ax.text(0.5, 0.5, value_str, ha="center", va="center", fontsize=24, fontweight="bold", transform=ax.transAxes)
    ...
    for i, chart_config in enumerate(charts):
        ...
        ax = fig.add_axes(
            [0.05 + col * 0.47, chart_start_y - (row + 1) * chart_height + 0.02, 0.43, chart_height - 0.04]
        )
        ...
        if chart_type == "bar":
            ...
            ax.bar(subset[chart_config["x"]], subset[chart_config["y"]], color="#2E86AB", edgecolor="black", alpha=0.8)
            ...
            subset = data[[chart_config["x"], chart_config["y"]]].dropna()
            ax.plot(subset[chart_config["x"]], subset[chart_config["y"]], color="#2E86AB", linewidth=2, marker="o")
            ax.set_xlabel(chart_config["x"])
            ax.set_ylabel(chart_config["y"])
            plt.setp(ax.xaxis.get_majorticklabels(), rotation=45, ha="right")
...
def main():
    ...
    parser.add_argument(
        "--plot-type", default="hist", choices=["hist", "kde", "box", "violin"], help="Distribution plot type"
    )
    ...
    try:
        if args.type == "bar":
            ...
            fig = create_distribution_plot(
                data, args.x, args.hue, args.title, args.xlabel, args.palette, plot_type=args.plot_type
            )
