...
### 2.1 入力契約
`target_profile.yaml`（雛形: `assets/target_profile_template.yaml`、仕様: `references/input_contract.md`、対話生成: `references/interview_wizard.md` + `scripts/build_target_profile.py`、回答例: `assets/wizard_answers_example.json`）
...
- **既存 target_profile.yaml が提供されている場合** → §2.2 の整合性検証へ
- **未提供の場合（対話ウィザード）** → `references/interview_wizard.md` の 7 段階に従い、
  `AskUserQuestion` を順次発行して回答を `wizard_answers.json` に集約 →
  `scripts/build_target_profile.py --answers wizard_answers.json --output <evidence_dir>/target_profile.yaml`
  で生成。
  - Stage 1 で `os_family=windows` または `web_server=iis` を検知した時点で**以降の質問を打ち切り、unsupported メッセージを返す**（§1.3 hard fail）
  - 対話で聞くのは **判断・本人確認・承認・スコープ確定** に必要な項目のみ。既定値・派生値・空配列は build_target_profile.py が補完
  - script 終了コード: `0`=正常 / `1`=入力検証エラー（該当 stage を再質問）/ `2`=I/O エラー / `3`=hard fail
...
- `references/interview_wizard.md`: target_profile.yaml 対話収集仕様（7 stages）
- `scripts/build_target_profile.py`: wizard_answers.json → target_profile.yaml 生成
- `assets/wizard_answers_example.json`: ウィザード回答サンプル
- `assets/`: 各種テンプレート
