...
class FinancialAnalyzer:
    ...
    def sensitivity_analysis(
        base_investment: float, base_annual_benefit: float, years: int, discount_rate: float = 0.10
...
class BusinessMetrics:
    ...
    @staticmethod
    def calculate_cltv(avg_purchase_value: float, purchase_frequency: float, customer_lifespan: float) -> float:
...
class ComparativeAnalyzer:
    ...
    def weighted_scoring(
        ...
        for option_name, scores in options.items():
            weighted_score = sum(scores[criterion] * weights[criterion] for criterion in weights.keys())
            results[option_name] = {"weighted_score": round(weighted_score, 2), "detail": scores}
...
def main():
    parser = argparse.ArgumentParser(
        description="Business Analysis Toolkit", formatter_class=argparse.RawDescriptionHelpFormatter, epilog=__doc__
    )
    ...
    financial_parser.add_argument(
        "--discount-rate", type=float, default=10, help="Discount rate percentage (default: 10)"
    )
