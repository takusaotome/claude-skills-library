...
- **Python 3.8+** with optional `numpy` (for enhanced numerical operations)
- **Input data in JSON format** for valuation calculator scripts
- Financial data for target company (FCF projections, WACC parameters, etc.)
- Comparable company data for multiples analysis

## Output

This skill generates the following outputs:

| Workflow | Output | Format |
|----------|--------|--------|
| DD実施 | DDレポート | Markdown (`assets/dd_report_template_*.md`) |
| バリュエーション | 企業価値評価サマリー | Markdown (`assets/valuation_summary_template_*.md`) |
| シナジー分析 | シナジーモデル | Markdown (`assets/synergy_model_template_*.md`) |
| PMI計画 | PMI計画書 | Markdown (`assets/pmi_plan_template_*.md`) |

**Valuation Calculator Script Outputs:**
- WACC calculation report (Markdown)
- DCF valuation analysis with sensitivity matrix (Markdown)
- Comparable companies analysis with implied valuations (Markdown)
- Synergy NPV analysis (Markdown)
...
```bash
# WACC計算
python scripts/valuation_calculator.py wacc wacc_params.json --output wacc_report.md

# DCFバリュエーション
python scripts/valuation_calculator.py dcf target_projections.json --output dcf_report.md

# 類似企業比較分析
python scripts/valuation_calculator.py multiples target.json comparable_companies.json --output comps_report.md

# シナジーNPV計算
python scripts/valuation_calculator.py synergy synergy_model.json --discount-rate 0.10 --output synergy_report.md

# 感度分析マトリックス
python scripts/valuation_calculator.py sensitivity model.json --steps 5 --output sensitivity_matrix.md
```
