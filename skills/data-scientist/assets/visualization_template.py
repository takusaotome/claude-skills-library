...
def plot_distribution(data, column, title=None):
    ...
    # Histogram with KDE
    axes[0].hist(
        data[column].dropna(), bins=30, edgecolor="black", alpha=0.7, density=True, color=COLORS_QUALITATIVE[0]
    )
    data[column].plot(kind="density", ax=axes[0], linewidth=2, color=COLORS_QUALITATIVE[1])
...
def plot_categorical_distribution(data, column, top_n=10, title=None):
    ...
    # Bar chart
    value_counts.plot(kind="bar", ax=axes[0], color=COLORS_QUALITATIVE[0], edgecolor="black", alpha=0.7)
    axes[0].set_xlabel(column)
    ...
    # Pie chart
    value_counts.plot(kind="pie", ax=axes[1], autopct="%1.1f%%", startangle=90, colors=COLORS_QUALITATIVE)
    axes[1].set_ylabel("")
...
def plot_scatter_with_regression(data, x_col, y_col, hue=None, title=None):
    ...
    if hue:
        for category in data[hue].unique():
            ...
            ax.scatter(data[mask][x_col], data[mask][y_col], label=category, alpha=0.6, s=50)
    else:
        ax.scatter(data[x_col], data[y_col], alpha=0.6, s=50, color=COLORS_QUALITATIVE[0])

        # Add regression line
        z = np.polyfit(data[x_col].dropna(), data[y_col].dropna(), 1)
        p = np.poly1d(z)
        ax.plot(data[x_col], p(data[x_col]), "r--", linewidth=2, label=f"Trend: y={z[0]:.2f}x+{z[1]:.2f}")
...
def plot_pairplot(data, hue=None, vars=None):
    ...
    g = sns.pairplot(data[vars + ([hue] if hue else [])], hue=hue, diag_kind="kde", height=3, plot_kws={"alpha": 0.6})
    g.fig.suptitle("Pairwise Relationships", y=1.01, fontsize=14, fontweight="bold")
    return g.fig
...
def plot_time_series(data, date_col, value_col, title=None):
    ...
    # Original series
    ax.plot(data[date_col], data[value_col], label="Original", linewidth=1.5, alpha=0.7, color=COLORS_QUALITATIVE[0])

    # Moving averages
    if len(data) >= 30:
        ma_30 = data[value_col].rolling(window=30).mean()
        ax.plot(data[date_col], ma_30, label="30-period MA", linewidth=2, color=COLORS_QUALITATIVE[1])

    if len(data) >= 90:
        ma_90 = data[value_col].rolling(window=90).mean()
        ax.plot(data[date_col], ma_90, label="90-period MA", linewidth=2, color=COLORS_QUALITATIVE[2])
...
def plot_seasonal_decomposition(data, value_col, period=12):
    ...
    decomposition = seasonal_decompose(data[value_col], model="additive", period=period)
...
def plot_roc_curve(y_true, y_pred_proba, title="ROC Curve"):
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.plot(fpr, tpr, linewidth=2, label=f"ROC curve (AUC = {roc_auc:.2f})", color=COLORS_QUALITATIVE[0])
...
def plot_feature_importance(feature_names, importances, top_n=20, title="Feature Importance"):
    ...
    ax.barh(range(len(importance_df)), importance_df["importance"], color=COLORS_QUALITATIVE[0], alpha=0.7)
...
def plot_residuals(y_true, y_pred, title="Residual Analysis"):
    ...
    axes[1].hist(residuals, bins=30, edgecolor="black", alpha=0.7, color=COLORS_QUALITATIVE[1])
...
def plot_learning_curves(train_sizes, train_scores, val_scores, title="Learning Curves"):
    ...
    ax.plot(train_sizes, train_mean, label="Training score", linewidth=2, color=COLORS_QUALITATIVE[0])
    ax.fill_between(train_sizes, train_mean - train_std, train_mean + train_std, alpha=0.2, color=COLORS_QUALITATIVE[0])

    ax.plot(train_sizes, val_mean, label="Validation score", linewidth=2, color=COLORS_QUALITATIVE[1])
    ax.fill_between(train_sizes, val_mean - val_std, val_mean + val_std, alpha=0.2, color=COLORS_QUALITATIVE[1])
