        ...
        for name, model in models.items():
            ...
            if self.problem_type == "regression":
                cv_scores = cross_val_score(
                    model, self.X_train, self.y_train, cv=cv, scoring="neg_mean_squared_error", n_jobs=-1
                )
            ...
            if self.problem_type == "regression":
                ...
                print(
                    f"  CV RMSE: {cv_rmse:.4f} | Test RMSE: {metrics['Test_RMSE']:.4f} | "
                ...
                if len(np.unique(self.y_train)) == 2 and y_pred_proba is not None:
                    ...
                    print(
                        f"  CV F1: {cv_score:.4f} | Test Acc: {metrics['Test_Accuracy']:.4f} | "
    ...
    def generate_comparison_report(self):
        ...
        if self.problem_type == "regression":
            ...
            for idx, row in results_df.iterrows():
                axes[2].annotate(row["Model"], (row["Test_Recall"], row["Test_Precision"]), fontsize=8, alpha=0.7)
            ...
            plt.tight_layout()
            plt.savefig(
                self.output_dir / f"confusion_matrix_{best_model_name.replace(' ', '_')}.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
...
Examples:
  ...
  python model_comparison.py sales.xlsx revenue --output my_results/
    ...
    parser.add_argument("--output", "-o", default="model_comparison", help="Output directory")
    ...
    comparison = ModelComparison(args.data_file, args.target, problem_type=args.problem_type, output_dir=args.output)
