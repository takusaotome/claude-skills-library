...
class TimeSeriesAnalysis:
    ...
    def test_stationarity(self):
        ...
        self._add_to_report("  Critical Values:")
    ...
    def forecast(self, periods=30):
        ...
        try:
            model_es = ExponentialSmoothing(
                train, seasonal_periods=min(12, len(train) // 2), trend="add", seasonal="add"
            )
            fit_es = model_es.fit()
...
Examples:
  ...
  python timeseries_analysis.py data.csv value --output ts_results/
    ...
    parser.add_argument(
        "--forecast-periods", "-f", type=int, default=30, help="Number of periods to forecast (default: 30)"
    )
    parser.add_argument("--output", "-o", default="timeseries_analysis", help="Output directory")
    ...
    analysis = TimeSeriesAnalysis(args.data_file, args.value_col, date_col=args.date_col, output_dir=args.output)
