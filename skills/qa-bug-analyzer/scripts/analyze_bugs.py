...
class BugAnalyzer:
    ...
    def analyze_severity_distribution(self) -> Dict[str, Any]:
        ...
        distribution = {
            severity: {"count": count, "percentage": round(count / total * 100, 2) if total > 0 else 0}
            for severity, count in severity_counts.most_common()
        }

        return {"distribution": distribution, "total_bugs": total, "unique_severities": len(severity_counts)}
    ...
    def analyze_category_distribution(self) -> Dict[str, Any]:
        ...
        distribution = {
            category: {"count": count, "percentage": round(count / total * 100, 2) if total > 0 else 0}
            for category, count in category_counts.most_common()
        }

        return {"distribution": distribution, "total_bugs": total, "top_categories": category_counts.most_common(5)}
    ...
    def analyze_module_distribution(self) -> Dict[str, Any]:
        ...
        distribution = {
            module: {"count": count, "percentage": round(count / total * 100, 2) if total > 0 else 0}
    ...
    def analyze_resolution_time(self) -> Dict[str, Any]:
        ...
        if not resolution_days:
            return {"total_resolved": 0, "message": "No resolution time data available"}
    ...
    def analyze_status_distribution(self) -> Dict[str, Any]:
        ...
        open_count = sum(count for status, count in status_counts.items() if status.lower() in open_statuses)
        closed_count = sum(count for status, count in status_counts.items() if status.lower() in closed_statuses)
    ...
    def generate_recommendations(self) -> List[Dict[str, Any]]:
        ...
        for issue in quality_issues:
            recommendations.append(
                {
                    "priority": "High"
                    if "critical" in issue["issue"].lower() or "severe" in issue["issue"].lower()
                    else "Medium",
                    "area": issue["area"],
...
def main():
    parser = argparse.ArgumentParser(description="Analyze bug ticket data for quality management insights")
    parser.add_argument("input_file", help="Path to bug data file (CSV or JSON)")
    parser.add_argument("--output", "-o", help="Output file path for JSON results (optional)", default=None)
    parser.add_argument(
        "--format",
        "-f",
