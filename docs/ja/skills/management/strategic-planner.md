...
[スキルパッケージをダウンロード (.skill)](https://github.com/takusaotome/claude-skills-library/raw/main/skill-packages/strategic-planner.skill){: .btn .btn-primary .fs-5 .mb-4 .mb-md-0 .mr-2 }
[GitHubでソースを見る](https://github.com/takusaotome/claude-skills-library/tree/main/skills/strategic-planner){: .btn .fs-5 .mb-4 .mb-md-0 }
...
状況に応じて適切なフレームワークを選択します。

| やりたいこと | 推奨フレームワーク | 理由 |
|:-----------|:----------------|:----|
| マクロトレンド（規制・経済・人口動態）を理解したい | PEST / PESTLE | 業界レベルを超えた外部環境要因をスキャン |
| 業界の収益性と競争圧力を評価したい | Porter's Five Forces | マージンを決定する構造的な力をマッピング |
| 自社の強み・弱みを特定したい | Value Chain | 組織内で価値が創造・毀損される箇所を特定 |
| アクション可能な戦略オプションを生成したい | Cross-SWOT | 内部・外部要因を4つの戦略象限に統合 |
| ビジネスモデルを設計・再設計したい | Business Model Canvas / Lean Canvas | ビジネスロジック全体を1枚で可視化 |
| 複数事業への投資配分を決めたい | BCG Matrix / GE Matrix | 成長ポテンシャルと競争ポジションで事業を分類 |
| 成長方向を選択したい | Ansoff Matrix | 市場浸透・市場開発・製品開発・多角化のトレードオフを明確化 |

### データに関する注意事項

戦略分析の精度は、裏付けとなるデータの質に依存します。

- **SWOT項目はエビデンスベースであるべき。** 「ブランド力が強い」ではなく「ターゲット層でのブランド認知78%（業界平均45%）」のように測定可能な表現にしましょう。根拠のない主張は分析を弱めます。
- **Porter's Five Forcesには市場データが必要。** 業界集中度、スイッチングコストの推定値、代替品の価格データがなければ、評価は意見にとどまります。業界レポートや公開情報を可能な限り参照してください。
- **BCGとGEマトリクスは信頼できる市場規模データに依存。** 成長率と市場シェアの数値は信頼性の高いソース（業界団体、アナリストレポート）から取得してください。推定値にはその旨を明記しましょう。
- **中期計画の財務予測は仮説であり予測ではない。** 楽観・基本・悲観の3シナリオによる感度分析を必ず含め、前提条件を明示してください。
- **Claudeは提供された情報に基づいて分析を生成します。** インプットが不完全または偏りがあれば、出力もそれを反映します。一次調査（顧客インタビュー、競合情報、財務データ）でClaudeの分析を補完してください。

### リファレンスファイルとテンプレート

3つのリファレンスガイドと3つの出力テンプレートが同梱されています。

| ファイル | 種類 | 内容 |
|:--------|:-----|:----|
| `references/strategic_analysis_frameworks.md` | リファレンス | PEST/Porter/Value Chainの詳細手法 |
| `references/business_model_frameworks.md` | リファレンス | BMC、Lean Canvas、Value Proposition Canvasガイド |
| `references/portfolio_matrix_frameworks.md` | リファレンス | BCG、GE/McKinsey、Ansoffの詳細手順 |
| `assets/strategic_plan_template.md` | テンプレート | 中期経営計画（13セクション） |
| `assets/new_business_proposal_template.md` | テンプレート | 新規事業企画書 |
| `assets/portfolio_analysis_template.md` | テンプレート | マトリクス付きポートフォリオ分析レポート |
...
### 例4: コングロマリットのポートフォリオ分析

```
4つの事業部門があります:
A) エンタープライズソフトウェア（高成長、市場リーダー）
B) レガシーハードウェア（横ばい成長、シェア1位）
C) クラウドサービス（高成長、市場4位）
D) 印刷サービス（縮小傾向、小規模シェア）
BCG分析とAnsoff分析を実施し、投資配分を提言してください。
```

### 例5: プラットフォームビジネスのBMC

```
製造業者と物流業者をつなぐB2Bマーケットプレイスを構築中です。
9ブロックすべてをカバーするビジネスモデルキャンバスを作成し、
主要な収益モデルオプション（取引手数料、サブスクリプション、
ハイブリッド）を特定してください。
```
...
### SWOT分析が曖昧でアクションにつながらない

**症状**: 「良いチーム」「市場機会あり」など、具体性のないSWOT項目が並ぶ。

**解決策**: 財務指標、市場シェア、顧客調査結果、競合ベンチマークなどの具体的なデータをClaudeに提供します。各SWOT項目を測定可能な指標で書き直すよう依頼してください。データがない場合は「(推定)」とラベル付けし、検証のための一次調査を計画します。

### 戦略計画の施策が多すぎる

**症状**: 計画に15-20の施策が並び、明確な優先順位がない。

**解決策**: 加重スコアリングマトリクス（戦略適合性・実現可能性・インパクト・緊急性）を適用し、上位5-7施策に絞るようClaudeに依頼します。「施策を7以内に絞り込み、除外したものとその理由を説明して」と指示してください。

### ポートフォリオ分析が主観的に感じる

**症状**: BCGやGEマトリクスの配置が評価者の判断に大きく依存している。

**解決策**: 分析開始前に評価基準とウェイトを定義します。ステークホルダーと合意を取ります。業界レポートの市場成長率や売上シェアなど、定量データを可能な限り使用します。推定値が必要な場合はその旨を明記し、配置に対する感度分析を実施します。
