...
Design Implementation Reviewer に関する日本語ガイドです。`skills/design-implementation-reviewer/SKILL.md` をもとに、利用開始手順、参照ファイル、補助スクリプトへの入口を日本語で整理しています。
{: .fs-6 .fw-300 }
...
このページは **Design Implementation Reviewer** スキルの日本語サマリーです。
- スキル本体: `skills/design-implementation-reviewer/SKILL.md`
- 参照ガイド: 2 件
- 補助スクリプト: なし
...
1. `skills/design-implementation-reviewer/SKILL.md` を開き、対象タスクと期待する成果物を確認します。
...
- **Design Implementation Reviewer** に沿って作業の進め方を整理したいとき
...
- 詳細な手順や判断基準は `skills/design-implementation-reviewer/SKILL.md` を基準にしてください。
...
- 詳細な解説、背景説明、個別の運用判断は [English version]({{ '/en/skills/dev/design-implementation-reviewer/' | relative_url }}) を参照してください。
